...
dTitleText.Text = library.LoadTitle or "Asset"
...
-- Mobile-optimized and bug-fixed slider
function sectiontable:Slider(Info)
    ...
    sliderButton.Size = UDim2.new(0, 149, 0, 14) -- Enlarged hitbox for mobile
    sliderButton.Parent = slider

    local function updateSlider(px)
        local clamped = math.clamp(px, 0, 1)
        local value = math.floor(Info.Minimum + ((Info.Maximum - Info.Minimum) * clamped))
...
function sectiontable:Dropdown(Info)
    ...
    dropdownPadding.PaddingTop = UDim.new(0, 27)
    ...
    function insidedropdown:Add(text)
        ...
        clickButton.MouseEnter:Connect(function()
            TweenService:Create(label, TweenInfo.new(.15), {TextColor3 = Color3.fromRGB(255, 255, 255)}):Play()
        end)

        clickButton.MouseLeave:Connect(function()
            TweenService:Create(label, TweenInfo.new(.15), {TextColor3 = Color3.fromRGB(191, 191, 191)}):Play()
        end)
...
function sectiontable:MultiDropdown(Info)
    ...
    Info.MaxSelected = Info.MaxSelected or 1
    ...
    local label = Instance.new("TextLabel")
    label.Name = "MultiDropdownText"
    label.Font = Enum.Font.GothamBold
    label.Text = Info.Text
    label.TextColor3 = Color3.fromRGB(217, 217, 217)
    label.TextSize = 11
    label.TextXAlignment = Enum.TextXAlignment.Left
    label.BackgroundTransparency = 1
    label.Position = UDim2.new(0.0488, 0, 0, 0)
    ...
    icon.Name = "MultiDropdownIcon"
    icon.Image = getcustomasset("UI_Phosphorus/CollapseArrow.png")
    icon.ImageColor3 = Color3.fromRGB(191, 191, 191)
    icon.AnchorPoint = Vector2.new(1, 0)
    icon.BackgroundTransparency = 1
    icon.Rotation = -90
    icon.Position = UDim2.new(0, 155, 0, 7)
    icon.Size = UDim2.new(0, 13, 0, 13)
    icon.ZIndex = 2
    icon.Parent = dropdown

    local toggleBtn = Instance.new("TextButton")
    toggleBtn.Name = "MultiDropdownToggle"
    toggleBtn.Text = ""
    ...
    local function toggleOption(optionButton, labelText, optionName)
        ...
        if Info.Flag then library.Flags[Info.Flag] = selected end
    end
    ...
    local function addOption(optionName)
        ...
        textLabel.BackgroundTransparency = 1
        textLabel.Position = UDim2.new(0.0488, 0, 0, 0)
        textLabel.Size = UDim2.new(0, 156, 0, 27)
    ...
    for _, option in ipairs(Info.List) do
        addOption(option)
    end
